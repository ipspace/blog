...
<!--more-->
In the next moment, you'll likely experience an extremely painful collision with reality, shattering most PowerPoint fairy  tales. The gray box labeled *Azure VNet* is really a router (see [Azure Networking 101](/2020/05/azure-networking-101/) for details), and a router won't forward packets unless it has *routes to the destinations* ([details](/2021/03/video-path-discovery-bridging-routing/)).

{{<figure src="/2021/03/azure-rs-virtual-router.png">}}

To make your design work, you'd have to run a routing protocol between your SD-WAN appliances and the Azure virtual router (ignoring for the moment the *[we don't need routing protocols in SD-WAN](/2015/06/software-defined-wanwell-orchestrated/)* stupidities)... but you can't. 
...
Cisco hyped the first solution a while ago: deploy two CSR instances in a separate virtual network, and establish VPN tunnels with the target virtual networks. [Clumsy, slow, and expensive](/2018/09/using-csr1000v-in-aws-instead-of/). This time you can have all three.
...
* Start with [Azure Networking 101](/2020/05/azure-networking-101/) blog post.
